...
const useFetchPhotosUP = (searchField:string = 'Cupertino'):{data:{photos:Result[],loading:boolean},loadPlaces:Function} => {
  ...
  const loadPlaces = async () => {
    ...
    const photos = await getPhotosUnsplash(searchField);
...
const getPhotosUnsplash = async (searchField:string):Promise<Result[]> => {
