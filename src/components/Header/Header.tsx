...
const MenuButton:FC<{setIsOpen:Function, isOpen:boolean}> = ({setIsOpen,isOpen}) => {
  return (
    <div className="flex">
      <button type='button' onClick={ () => setIsOpen(!isOpen)} className='px-2 text-gray-500 hover:text-white focus:outline-none focus:text-white'>
        <svg
          className="h-6 w-6 fill-current"
          xmlns="http://www.w3.org/2000/svg"
          viewBox="0 0 24 24"
        >
        <path className={`${isOpen&&'hidden'}`} v-if="!isOpen" fillRule="evenodd" d="M4 5h16a1 1 0 0 1 0 2H4a1 1 0 1 1 0-2zm0 6h16a1 1 0 0 1 0 2H4a1 1 0 0 1 0-2zm0 6h16a1 1 0 0 1 0 2H4a1 1 0 0 1 0-2z" />
        <path className={`${!isOpen&&'hidden'}`} v-if="isOpen" fillRule="evenodd" clipRule="evenodd" d="M18.278 16.864a1 1 0 0 1-1.414 1.414l-4.829-4.828-4.828 4.828a1 1 0 0 1-1.414-1.414l4.828-4.829-4.828-4.828a1 1 0 0 1 1.414-1.414l4.829 4.828 4.828-4.828a1 1 0 1 1 1.414 1.414l-4.828 4.829 4.828 4.828z"/>
...
const NavBar:FC<{isOpen:boolean}> = ({isOpen}) => {
  const links = [
    {
  ...
  return (
    <nav className={`${!isOpen&&'hidden'}`}>
      ...
      <div className='px-5 py-5 '>
          ...
          <div className='mt-5 '>
            {
                ProfileLinks.map( ({link}) => <Link className='block mt-3 first:mt-0 text-gray-400 hover:text-white' key={link} to={"#"}>{link}</Link> )
            }
          </div>
      </div>
    </nav>
